...
for sample in ${DIR_Bams}/*.bam; do
	...
	if [ -n "$INPUT" ]; then # with input  
	    qsub -q public.q -o $cwd/logs -j yes -pe smp $nb_cores -cwd -b y -shell y -N macs2 "module load macs/2.1.0; macs2 callpeak -t $sample -c $INPUT -n ${out}_macs2_pval_${pval} -f BAM -g $species_macs --call-summits" 
	
	else # without input
	    if [ "$format" == "BAMPE" ]; then
		qsub -q public.q -o $cwd/logs -j yes -pe smp $nb_cores -cwd -b y -shell y -N macs2 "module load macs/2.1.0; macs2 callpeak -t $sample -n ${out}_macs2_pval_${pval} -f BAM -g $species_macs --nomodel --shift -100 --extsize 200 --keep-dup all --call-summits"
	    else
		qsub -q public.q -o $cwd/logs -j yes -pe smp $nb_cores -cwd -b y -shell y -N macs2 "module load macs/2.1.0; macs2 callpeak -t $sample -n ${out}_macs2_pval_${pval} -f BAM -g $species_macs --nomodel --extsize 200 --keep-dup all --call-summits" 
	    fi
	fi
	cd $cwd
    fi
